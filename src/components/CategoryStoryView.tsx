            ...
            {isExpanded && (
              <div className="bg-card rounded-b-3xl border border-t-0 border-border/30 px-6 py-5 animate-fade-in space-y-5">
                {card.photos.length > 0 && (
                  <div>
                    <span className="text-xs font-semibold text-foreground mb-2.5 block">{t("catStory.photos")}</span>
                    <div className="grid grid-cols-3 gap-2.5">
                      {card.photos.slice(0, 6).map((photo, pi) => (<div key={pi} className="aspect-square rounded-2xl overflow-hidden bg-muted/30 shadow-sm"><img src={photo} alt="" className="w-full h-full object-cover" /></div>))}
                    </div>
                    {card.photos.length > 6 && <p className="text-[10px] text-muted-foreground mt-1.5 text-center">{interpolate(t("catStory.morePhotos"), { n: card.photos.length - 6 })}</p>}
                  </div>
                )}
                <div>
                  <span className="text-xs font-semibold text-foreground mb-2.5 block">{t("catStory.timeline")}</span>
                  <div className="space-y-2 max-h-44 overflow-y-auto">
                    {card.completed.sort((a, b) => (b.date?.getTime() || 0) - (a.date?.getTime() || 0)).slice(0, 10).map((task, ti) => (
                      <div key={ti} className="flex items-center gap-2.5 px-3.5 py-2 bg-muted/20 rounded-xl text-xs">
                ...
                {story && (
                  <div className="p-4 rounded-2xl bg-muted/15 border border-border/10">
                    <div className="flex items-center gap-2 mb-2"><span className="text-lg">{story.emoji}</span><span className="text-sm font-bold text-foreground">{story.title}</span></div>
                    <p className="text-xs text-primary italic mb-2.5">"{story.openingLine}"</p>
                ...
                <button onClick={() => generateCategoryStory(card)} disabled={isLoading}
                  className={cn("w-full flex items-center justify-center gap-2 py-3 rounded-2xl text-xs font-medium transition-all", story ? "bg-muted/40 text-muted-foreground hover:text-foreground" : "bg-primary/10 text-primary hover:bg-primary/20")}>
                ...
                {story && (
                  <button onClick={() => {
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2.5 rounded-2xl bg-muted/25 text-muted-foreground hover:text-primary text-xs font-medium transition-all">

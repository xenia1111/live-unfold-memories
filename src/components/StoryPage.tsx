            ...
            <Carousel setApi={setCarouselApi} className="w-full" opts={{ align: "start", loop: false }}>
              <CarouselContent className="-ml-0">
                {cards.map((card, storyIndex) => {
                  ...
                  return (
                    <CarouselItem key={card.key} className="pl-0 basis-full">
                        ...
                        <div className={cn("rounded-3xl overflow-hidden shadow-sm border-2 border-foreground/8 relative", card.decor.bg)}>
                          ...
                          <div className="flex justify-between px-4 py-1.5">
                            {Array.from({ length: 8 }).map((_, i) => (
                              <div key={i} className="w-2.5 h-2.5 rounded-sm bg-foreground/6" />
                            ))}
                          </div>

                          {/* Story header area */}
                          <div className="px-7 pt-4 pb-5 relative">
